...
final class BitmapLoadingWorkerTask extends AsyncTask<Void, Void, BitmapLoadingWorkerTask.Result> {
     ...
     */
    @Override
    protected BitmapLoadingWorkerTask.Result doInBackground(Void... params) {
        try {
            if (!isCancelled()) {
                ...
                if (!isCancelled()) {
                    ...
                    return new BitmapLoadingWorkerTask.Result(
                            mUri, rotateResult.bitmap, decodeResult.sampleSize, rotateResult.degrees);
                }
            }
            return null;
        } catch (Exception e) {
            return new BitmapLoadingWorkerTask.Result(mUri, e);
        }
    }
     ...
     */
    @Override
    protected void onPostExecute(BitmapLoadingWorkerTask.Result result) {
        if (result != null) {
