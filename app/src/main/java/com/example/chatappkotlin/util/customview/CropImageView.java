...
public class CropImageView extends FrameLayout {
    ...
    private CropImageView.ScaleType mScaleType;
    ...
    private CropImageView.OnSetCropOverlayReleasedListener mOnCropOverlayReleasedListener;

    /** callback to be invoked when crop overlay is moved. */
    private CropImageView.OnSetCropOverlayMovedListener mOnSetCropOverlayMovedListener;

    /** callback to be invoked when crop window is changed. */
    private CropImageView.OnSetCropWindowChangeListener mOnSetCropWindowChangeListener;

    /** callback to be invoked when image async loading is complete. */
    private CropImageView.OnSetImageUriCompleteListener mOnSetImageUriCompleteListener;

    /** callback to be invoked when image async cropping is complete. */
    private CropImageView.OnCropImageCompleteListener mOnCropImageCompleteListener;
        ...
        if (options == null) {
            ...
            if (attrs != null) {
                ...
                try {
                    ...
                    options.scaleType =
                            CropImageView.ScaleType.values()[
                    ...
                    options.guidelines =
                            CropImageView.Guidelines.values()[
                                    ta.getInt(
    ...
    public Bitmap getCroppedImage() {
        return getCroppedImage(0, 0,CropImageView.RequestSizeOptions.NONE);
    }


    public Bitmap getCroppedImage(int reqWidth, int reqHeight) {
        return getCroppedImage(reqWidth, reqHeight, CropImageView.RequestSizeOptions.RESIZE_INSIDE);
    }
     ...
     * @return a new Bitmap representing the cropped image
     */
    public Bitmap getCroppedImage(int reqWidth, int reqHeight, CropImageView.RequestSizeOptions options) {
        Bitmap croppedBitmap = null;
        if (mBitmap != null) {
            mImageView.clearAnimation();

            reqWidth = options != CropImageView.RequestSizeOptions.NONE ? reqWidth : 0;
            reqHeight = options !=CropImageView.RequestSizeOptions.NONE ? reqHeight : 0;

            if (mLoadedImageUri != null
                    && (mLoadedSampleSize > 1 || options == CropImageView.RequestSizeOptions.SAMPLING)) {
    ...
    public void getCroppedImageAsync() {
        getCroppedImageAsync(0, 0, CropImageView.RequestSizeOptions.NONE);
    }
     ...
     * #setOnCropImageCompleteListener(CropImageView.OnCropImageCompleteListener)}.
     *
    ...
    public void getCroppedImageAsync(int reqWidth, int reqHeight) {
        getCroppedImageAsync(reqWidth, reqHeight, CropImageView.RequestSizeOptions.RESIZE_INSIDE);
    }
     ...
     * @param options the resize method to use, see its documentation
     */
    public void getCroppedImageAsync(int reqWidth, int reqHeight, CropImageView.RequestSizeOptions options) {
    ...
    public void saveCroppedImageAsync(Uri saveUri) {
        saveCroppedImageAsync(saveUri, Bitmap.CompressFormat.JPEG, 90, 0, 0, CropImageView.RequestSizeOptions.NONE);
    }
    ...
    public void saveCroppedImageAsync(
        ...
        saveCroppedImageAsync(
                saveUri, saveCompressFormat, saveCompressQuality, 0, 0, CropImageView.RequestSizeOptions.NONE);
    }
    ...
    /** Set the callback t */
    public void setOnSetCropOverlayReleasedListener(CropImageView.OnSetCropOverlayReleasedListener listener) {
    ...
    void onSetImageUriAsyncComplete(BitmapLoadingWorkerTask.Result result) {
        ...
        CropImageView.OnSetImageUriCompleteListener listener = mOnSetImageUriCompleteListener;
        if (listener != null) {
    ...
    void onImageCroppingAsyncComplete(BitmapCroppingWorkerTask.Result result) {
       ...
       CropImageView.OnCropImageCompleteListener listener = mOnCropImageCompleteListener;
        if (listener != null) {
            CropImageView.CropResult cropResult =
                    new CropImageView.CropResult(
                            mBitmap,
                            mLoadedImageUri,
                            result.bitmap,
                            result.uri,
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null
                ...
                mCropOverlayView.setCropShape(CropImageView.CropShape.valueOf(bundle.getString("CROP_SHAPE")));
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (mScaleType == CropImageView.ScaleType.FIT_CENTER
                    || (mScaleType == CropImageView.ScaleType.CENTER_INSIDE && scale < 1)
